...
Command.prototype.parseExpectedArgs = function(args){
  ...
  args.forEach(function(arg){
    ...
    if (arg.indexOf('.')>-1) {
      argDef.name = arg.slice(1, -1).replace(/\.+/g, '');
      argDef.multi = true;
    } else {
      argDef.name = arg.slice(1, -1)
      argDef.multi = false;
    }
...
Command.prototype.parseOptions = function(argv){
  ...
  if (this.options) {
    this.options.forEach(function(option) {
      var found = foundOptions.filter(function(foundOption) {return (foundOption === option)})
      if (found.length == 0) {
        //option was not found.
        if (option.required && !self[option.name()]) {
          return self.missingOption(option.flags);
        }
      }
    })
  }
 ...
 * @api private
 */
Command.prototype.missingOption = function(name){
  console.error();
  console.error("  error: missing required option `%s'", name);
  console.error();
  process.exit(1);
};
